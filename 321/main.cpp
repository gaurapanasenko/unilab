...
void show_array(FILE * output,int * array,int length) {
	if (length == 0) {
		fprintf(output,"Массив порожній\n");
		return;
	}
	fprintf(output,"Массив має такі значення:\n");
	for (int i=0;i < length;i++) {
...
int main() {
	...
	while (length < 1 || length > 1000) {
		...
		if (length < 1 || length > 1000) {
			system("clear");
			printf("Вы ввели неправильну довжину, дозволено тільки ціле число від 1 до 1000\n");
		}
	}
	...
	do {
		...
		if (answer == 1) {
			...
			if (max == length) printf("Максимальний елемент знаходиться у кінці масива, тому вихідний масив порожній\n");
			else {
				int * b,lengthb;

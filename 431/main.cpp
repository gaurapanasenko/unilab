...
void keyboard(unsigned char theKey, GLint mouse_x, GLint mouse_y) {
	...
	switch (theKey) {
		...
		case 'd':
			if (active == -1)
				for (size_t i = 0; i < arr.size(); i++)
					arr[i].move_rel(Classes::Point(50, 0));
			else arr[active].move_rel(Classes::Point(50, 0));
			break;
		case 'a':
			if (active == -1)
				for (size_t i = 0; i < arr.size(); i++)

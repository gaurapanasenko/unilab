...
Програмно реалізувати алгоритми AES.

## Приклад роботи

### Консольний приклад

Скомпілюємо програму за допомогою команди `make`.

Створимо деякий ключ 128 біт (16 байт) та деяке повідомлення, та зашифруємо це повідомлення:
```shell
dd if=/dev/urandom of=key.bin count=1 bs=16
echo "Hello, world!" > data.txt
cat key.bin data.txt | ./main-128 > out.bin
```

Дешифруємо повідомлення та отримаємо наше повідомлення:
```shell
cat key.bin out.bin | ./main-128d
```
